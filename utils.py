...
def optimizer_scheduler(epoch, optimizer, p):
    """
    ...
    for param_group in optimizer.param_groups:
        ...
        param_group['weight_decay'] = 0.015
    return optimizer


def one_hot_embedding(labels, num_classes=31):
...
def visualize(encoder, training_mode, save_name):
    ...
    _, src_testset = get_dataset('amazon', path='database')
    _, tgt_testset = get_dataset('dslr', path='database')

    source_test_loader =torchdata.DataLoader(src_testset, batch_size=32, shuffle=True, num_workers=4, pin_memory=True, drop_last=True)
    target_test_loader = torchdata.DataLoader(tgt_testset, batch_size=32, shuffle=True, num_workers=4, pin_memory=True, drop_last=True)
    ...
    source_img_list = source_img_list.view(-1, 3, 227, 227)
    ...
    target_img_list = target_img_list.view(-1, 3, 227, 227)
...
def visualize_input():
    _, src_testset = get_dataset('amazon', 'database')
    _, tgt_testset = get_dataset('dslr', 'database')
    ...
    plot_embedding(dann_tsne, combined_label_list, combined_domain_list, 'input', 'amazon_n_dslr')
