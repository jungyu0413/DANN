...
def tester(encoder, classifier, discriminator, source_test_loader, target_test_loader, training_mode):
    ...
    for batch_idx, (source_data, target_data) in enumerate(zip(source_test_loader, target_test_loader)):
        count = min(len(source_test_loader), len(target_test_loader))
        p = float(batch_idx) / count
        ...
        # source_output = F.softmax(source_output, dim=0)
       ...
       # target_output = F.softmax(target_output, dim=0)
        ...
        if training_mode == 'dann':
            # Combined input -> Domain Classificaion
        ...
        #    domain_output = F.softmax(domain_output, dim=0)
            domain_pred = domain_output.max(1, keepdim=True)[1]
         #   print('domain_pred:', domain_pred.squeeze())
            domain_correct += domain_pred.eq(domain_combined_label.data.view_as(domain_pred)).cpu().sum()
         #   print('domain label: ', domain_combined_label) 
    ...
    if training_mode == 'dann':
        ...
        print('\nSource Accuracy: {}/{} ({:.2f}%)\n'
              ...
              'Domain Accuracy: {}/{} ({:.2f}%)\n'.
            format(
            source_correct, (count*batch_size), 100. * source_correct.item() / (count*batch_size),
            target_correct, (count*batch_size), 100. * target_correct.item() / (count*batch_size),
            domain_correct, (count + count)*batch_size, 100. * domain_correct.item() / ((count + count)*batch_size)
        ))
    else:
